...
func main() {
	...
	// Cloud Run instance metadata once — the worker pool (or service) name and revision from
	// environment variables and the unique instance ID from the GCP metadata server — then sets the
	// derived worker identity ("<instanceID>@<revision>") on the client, unless one is already set.
	// The metadata fetch performs a network request, so it never runs from workflow code. It fails
	// fast when the process is not running on a Cloud Run worker pool or service (for example, when
	// running locally), because the metadata server is unreachable.
	//
	...
	// The plugin fetched and cached the metadata during Dial. Read it back to log the identity it
	// applied to the client.
	md := plugin.Metadata()
	log.Printf("Worker identity: %s", md.WorkerIdentity())

	// The plugin only sets the client identity, so a zero worker.Options is all that is needed here.
